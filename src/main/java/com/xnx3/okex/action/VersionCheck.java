...
public class VersionCheck {
	public static void main(String[] args) throws IOException {
		check("http://version.xnx3.com/okexfuzhu.html", Global.VERSION, "okex");
	}
		...
		try {
			...
			if(!newVersion.equals(currentVersion)){
				...
				if(DialogUtil.showConfirmDialog("发现新版本 v"+newVersion+"， 是否更新最新版本？") == DialogUtil.CONFIRM_YES){
					if(com.xnx3.okex.util.SystemUtil.isWindowsOS()){
					...
					//其他系统，让他们到官网进行更新
					SystemUtil.openUrl(url);
				}
			}else{
				System.out.println("当前就是最新版本");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
