...
//		
		...
		//计算当前最低购买数量
		double buyMinSize = instrument.getMinSize() + instrument.getMinAddSize();
		...
		//分析完成，合适，可以自动购买
		System.out.println("当前最低价："+buyPn.getPrice()+" , 每次最小加价："+Public.getInstrument(instId).getMinAddPrice()+", 计算出来的购买价："+buyPrice+",  消耗USDT："+DoubleUtil.doubleToString(buyMinMoney));
